...
preferences {
	section("Choose which devices to monitor...") {
        ...
        input "cost", "attribute.cost", title: "Cost", multiple: true, required: false
...
def subscribeToEvents() {
	...
	if (energyMeters != null) {
        ...
        subscribe(energyMeters, "cost", genericHandler)
	}
    ...
    if (sensors != null) {
		subscribe(sensors, "value", genericHandler)
	}
...
def installed() {
	...
	atomicState.bucketKey = "SmartThings_ipwlx7gl" //change if needed
	atomicState.bucketName = "SmartThings_ipwlx7gl" //change if wanted
	atomicState.accessKey = "5pevfbVTAZOXsMXOqNewuJUOlwGTA7pc" //MUST CHANGE
